...
function KittyOnEvent(self, Event, arg1, arg2, arg3)
    ...
    if Event == "UNIT_POWER_UPDATE" and (arg1 == "player") and arg2 ==
        "COMBO_POINTS" then
    ...
    elseif Event == "UNIT_AURA" and
        (arg1 == "player" or (PetHasInterestingBuffs and arg1 == "pet")) then
        KittyOnBuffsChange()
...
function KittyInitialize()
    ...
    if (not VgerCore) or (not VgerCore.Version) or
        (VgerCore.Version < KittyVgerCoreVersionRequired) then
        if DEFAULT_CHAT_FRAME then
            DEFAULT_CHAT_FRAME:AddMessage("|cfffe8460" ..
                                              KittyLocal.NeedNewerVgerCoreMessage)
        end
...
function KittyCheckWayOfTheCrane()
    ...
    local configInfo = C_Traits.GetConfigInfo and
    ...
    for _, treeID in ipairs(treeIDs) do
        ...
        if nodes then
            for _, nodeID in ipairs(nodes) do
                ...
                if nodeInfo and nodeInfo.currentRank and nodeInfo.currentRank >
                    0 and nodeInfo.entryIDs then
                    for _, entryID in ipairs(nodeInfo.entryIDs) do
                        ...
                        if entryInfo and entryInfo.definitionID then
                            local defInfo =
                                C_Traits.GetDefinitionInfo and
                            ...
                            if defInfo and defInfo.spellID == 388779 then
                                ...
                                if nodeInfo.entryIDsWithCommittedRanks then
                                    for _, committedID in ipairs(
                                                              nodeInfo.entryIDsWithCommittedRanks) do
                                        if committedID == entryID then
                                            return true
                                        end
                                    end
                                    return false
...
function KittyOnBuffsChange()
    ...
    if BuffCharges ~= nil then -- BuffCharges will be 0 when the buff exists and nil when it isn't
        ...
        if BuffCharges ~= nil then KittyIsInArenaPreparation = true end
    end
    BuffCharges = nil -- otherwise we'll skip checks for the real buffs

    -- Frost mages' Icicles, only with Glacial Spike talent
    if BuffCharges == nil and VgerCore.IsMainline and Class == "MAGE" and Spec ==
        3 then
    ...
    if BuffCharges == nil and VgerCore.IsMainline and Class == "SHAMAN" and Spec ==
        2 then
    ...
    elseif BuffCharges == nil and (VgerCore.IsWrath or VgerCore.IsCataclysm) and
        Class == "SHAMAN" then
    ...
    if (BuffCharges > 0 or KittyEverHadBuffCharges) and
        (BuffCharges ~= KittyLastSoundPlayed) then
...
function KittyOnHolyPowerChange()
    ...
    if (HolyPowerCharges > 0 or KittyEverHadHolyPowerCharges) and
        (HolyPowerCharges ~= KittyLastSoundPlayed) then
        -- (No-op if the number actually hasn't changed.)
...
function KittyOnSoulShardsChange()
    ...
    if SoulShards ~= KittyLastSoundPlayed then
        -- (No-op if the number actually hasn't changed.)
        KittyCurrentMaxStacks =
            UnitPowerMax("player", Enum.PowerType.SoulShards)
        KittyThisResourceDecays = true
...
function KittyOnArcaneChargesChange()
    ...
    if ArcaneCharges ~= KittyLastSoundPlayed then
        -- (No-op if the number actually hasn't changed.)
        KittyCurrentMaxStacks = UnitPowerMax("player",
...
function KittyOnInsanityChange()
    ...
    if Insanity > 0 and KittyAuraStacks("player", "PLAYER HELPFUL", 194249) ~=
        nil then return end
...
function KittyOnAstralPowerChange()
    ...
    local _, Class = UnitClass("player")
    if Class ~= "DRUID" or not GetSpecialization or GetSpecialization() ~= 1 or
        GetShapeshiftFormID() == 1 then return end
    ...
    local SolarActive = KittyAuraStacks("player", "PLAYER HELPFUL", 48517) ~=
                            nil
    local LunarActive = KittyAuraStacks("player", "PLAYER HELPFUL", 48518) ~=
                            nil
...
function KittyOnComboPointsChange(Unit)
    ...
    if KittyDebug then
        VgerCore.Message("KITTYONCOMBOPOINTSCHANGE with Previous: " ..
                             KittyLastComboPoints .. ", now: " .. ComboPoints ..
                             ", last sound: " .. KittyLastSoundPlayed)
    end
    if (ComboPoints ~= KittyLastComboPoints) then
        -- (No-op if the number actually hasn't changed.)
        KittyCurrentMaxStacks = UnitPowerMax(Unit, Enum.PowerType.ComboPoints)
        VgerCore.Assert(
            KittyCurrentMaxStacks ~= nil and KittyCurrentMaxStacks > 0,
            "Hear Kitty: UnitPowerMax for combo points failed")
        KittyThisResourceDecays = true
...
function KittyFillMissingOptions()
    ...
    if KittyOptions.SoundPack == nil then
        KittyOptions.SoundPack = KittyDefaultSoundPackInternalName
    end
...
function KittyEnable(Enabled)
    VgerCore.Assert(Enabled == true or Enabled == false,
                    "New value should be true or false.")
...
function KittySetSoundChannel(Channel)
    VgerCore.Assert(Channel ~= nil,
                    "Usage: KittySetSoundChannel(\"Channel\"), where Channel can be Master, SFX, Music, or Ambience.")
...
function KittyEnableDoubleCrits(Double)
    VgerCore.Assert(Double == true or Double == false,
                    "New value should be true or false.")
...
function KittyEnableOnlyPlay5(OnlyPlay5)
    VgerCore.Assert(OnlyPlay5 == true or OnlyPlay5 == false,
                    "New value should be true or false.")
...
function KittyComboSound(ComboPoints)
    if KittyDebug then
        VgerCore.Message("KITTYCOMBOSOUND(" .. ComboPoints .. ") out of " ..
                             KittyCurrentMaxStacks .. " and last sound " ..
                             KittyLastSoundPlayed)
    end
    if ComboPoints < 0 then
        ComboPoints = 0
    elseif ComboPoints > KittyCurrentMaxStacks then
        ComboPoints = KittyCurrentMaxStacks
    end
    -- Don't replay the same sound we played last.
    if ComboPoints == KittyLastSoundPlayed then
        if KittyDebug then
            VgerCore.Message("Bailing out because we just played sound #" ..
                                 ComboPoints)
        end
        return
    end
    ...
    if KittyIsFirstBuffChange then
        ...
        if GetTime() < (KittyLoadTime + 30) then
            ...
            if KittyDebug then
                VgerCore.Message("Bailing out because we just started")
            end
            return
        end
    end
    ...
    if KittyOptions.DoubleCrits == false or KittyOptions.OnlyPlay5 then
        if (not KittyOptions.OnlyPlay5) or ComboPoints == 0 or ComboPoints ==
            KittyCurrentMaxStacks then KittyPlayOneSound(ComboPoints) end
        KittyLastSoundPlayed = ComboPoints
    ...
    if ComboPoints < KittyLastSoundPlayed then
        ...
        if KittyThisResourceDecays then
            ...
            if ComboPoints > 0 and KittyLastSoundPlayed - ComboPoints == 1 and
                ...
                if KittyDebug then
                    VgerCore.Message(
                        "Staying quiet because points decayed to " ..
                            ComboPoints)
                end
            else
                ...
                if KittyDebug then
                    VgerCore.Message("Playing single sound " .. ComboPoints)
                end
                KittyPlayOneSound(ComboPoints)
            end
        else
            if KittyDebug then
                VgerCore.Message("Playing a range from 0 to " .. ComboPoints)
            end
    ...
    elseif ComboPoints > KittyLastSoundPlayed then
        ...
        if KittyDebug then
            VgerCore.Message("Playing a range from " ..
                                 (KittyLastSoundPlayed + 1) .. " to " ..
                                 ComboPoints)
        end
        ...
        if KittyDebug then
            VgerCore.Message(
                "Didn't play anything because new combo points were last sound played")
        end
    end
...
function KittyPlayOneSound(ComboPoints, EvenIfOff)
    VgerCore.Assert(KittyCurrentMaxStacks,
                    "KittyCurrentMaxStacks should be set.")

    if (not EvenIfOff) and
        (KittyIsInArenaPreparation or not KittyOptions.Enabled) then return end
    ...
    local SoundKey = "Combo" .. KittyCurrentMaxStacks .. "StackSound" ..
                         ComboPoints
    ...
    if not Filename then
        ...
        if AltSoundKey then
            if KittyDebug then
                VgerCore.Message("[" .. ComboPoints .. "/" ..
                                     KittyCurrentMaxStacks ..
                                     "]  No sound defined to trying alternate:")
            end
    ...
    if not Filename then
        VgerCore.Fail("Hear Kitty has no sound defined for " ..
                          tostring(ComboPoints) .. "/" ..
                          tostring(KittyCurrentMaxStacks) ..
                          ".  Did an ability change?")
        Filename = "Combo5StackSound1"
    end
    if KittyDebug then
        VgerCore.Message("[" .. ComboPoints .. "/" .. KittyCurrentMaxStacks ..
                             "]  " .. Filename)
    end
...
function KittyAuraStacks(Unit, Filters, SpellID)
    -- On Midnight, only Monk currently has synthetic handling for in-combat aura restrictions.
    -- Keep legacy behavior for other classes by bailing out early.
    if VgerCore.IsMidnight then
        local _, Class = UnitClass("player")
    ...
    for i = 1, BUFF_MAX_DISPLAY do
        ...
        if issecretvalue and issecretvalue(AuraData.spellId) then
            return nil
        end -- Secret values in effect; can't compare
...
function KittyRegisterSoundPack(Name, Options)
    if not Name or not Options then
        VgerCore.Fail(
            "Usage: KittyRegisterSoundPack(\"Name\", { LocalizedName = \"Localized name\", ... })")
        return
    end
    if KittySoundPacks[Name] then
        VgerCore.Fail(
            "Couldn't install this Hear Kitty sound pack because another one with the name \"" ..
...
function KittySelectSoundPack(Name)
    if not KittySoundPacks[Name] then
        VgerCore.Fail(
            "Usage: KittySelectSoundPack(\"Name\").  Name is the internal name of the sound pack, not the localized name.")
        return
    end
...
function KittyGetSelectedSoundPackName()
    VgerCore.Assert(KittyOptions.SoundPack,
                    "KittyGetSelectedSoundPackName shouldn't be called before Hear Kitty finishes initializing.")
